...
/root/.profile && echo 'Ignore the previous error about stdin not being a tty. Fixing it now...') || exit 0;"
  ...
  config.vm.provision :shell, inline: "apt-get install --reinstall apt-transport-https ca-certificates && apt-add-repository ppa:webupd8team/java && apt-get update && apt-get -y install oracle-java8-installer curl xvfb ffmpeg x11-apps rxvt-unicode fonts-inconsolata xterm"
